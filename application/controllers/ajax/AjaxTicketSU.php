...
class AjaxTicketSU extends CI_Controller
{
    ...
    {
        $tickets = $this->db->query("SELECT tickets.pregunta, tickets.descripcion, tickets.porcentaje, tickets.prioridad,  tickets.id_mortal,
                         tickets.id_ticket FROM tickets WHERE tickets.id_ticket =" . $_POST['ticketid']);
        ...
        $estados = $this->db->query("SELECT ticketsu_tiene_estado.fecha_hora, estados.estado, estados.detalles, estados.id_estado FROM estados
                                        ...
                                        WHERE ticketsu_tiene_estado.id_ticketSU = ". $tickets[0]->id_ticket);
        ...
        $subTema = $this->db->query("SELECT id, nombre, id_tema FROM ticket_tiene_tema INNER JOIN sub_temas_ticket ON ticket_tiene_tema.idTema = id WHERE id_ticketSU = " . $tickets[0]->id_ticket);
